...
def draw_square(draw, x, y, zone_type):
    h = pzdzi.SQR_HEIGHT // 2
    w = pzdzi.SQR_WIDTH // 2
    draw.polygon([x, y - h, x + w, y, x, y + h, x - w, y], fill=FORAGING_COLOR[zone_type])
...
def render_foraging(dzi, tx, ty, cell_getter, out_path, save_empty):
    ...
    for gy in range(top, bottom + 1 - 6):
        for gx in range(left, right + 1):
            ...
            draw_square(draw, x, y, zone_type)
...
class CellGetter(object):
    def __init__(self, path):
        self.path = path
        self.getter = None

    def build_getter(self):
        self.cell_zones = pzobjects.load_cell_foraging_zones(self.path)
        getter = partial(pzobjects.square_map, self.cell_zones)
